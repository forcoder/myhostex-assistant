...
// ── 智能联想辅助函数 ──────────────────────────
/**
 * 简单模糊匹配：检查 query 中的每个字符是否按顺序出现在 text 中
 * 例如 "jage" 可以匹配到 "价格"
 */
...
/**
 * 文本高亮：用 <span class="kb-highlight"> 包裹匹配的部分
 */
function highlightText(text, search) {
  ...
  const idx = lowText.indexOf(lowSearch);
  if (idx === -1) return escaped;
  const before = escapeHtml(text.slice(0, idx));
  const match = escapeHtml(text.slice(idx, idx + search.length));
  const after = escapeHtml(text.slice(idx + search.length));
  return `${before}<span class="kb-highlight">${match}</span>${after}`;
}

/**
 * 对 KB 条目评分（用于联想排序）
 */
function scoreSuggestion(entry, search) {
  if (!search) return 0;
  const condition = (entry.trigger_condition || "").toLowerCase();
  const content = (entry.reply_content || "").toLowerCase();
  const props = (entry.applicable_properties || "").toLowerCase();
  let score = 0;

  // 关键词完全匹配最高分
  if (condition.includes(search)) score += 100;
  else if (fuzzyMatch(condition, search)) score += 50;

  // 内容匹配
  if (content.includes(search)) score += 30;
  else if (fuzzyMatch(content, search)) score += 15;

  // 房源匹配
  if (props.includes(search)) score += 20;

  // 启用状态加分
  if (entry.status === "启用") score += 10;

  // 触发次数加分
  if (entry.trigger_count > 100) score += 5;
...
// ── 智能联想下拉框 ──────────────────────────
function renderSmartSuggestions(search) {
  ...
  if (typeFilter) {
    candidates = candidates.filter((e) => e.trigger_type.includes(typeFilter));
  }

  const scored = candidates
    .map((e) => ({ entry: e, score: scoreSuggestion(e, search) }))
    .filter((x) => x.score > 0)
  ...
  sugEl.innerHTML = scored
    .map(
      (x, i) => `
      <div class="kb-sug-item" data-search="${escapeHtml(search)}" data-keyword="${escapeHtml(x.entry.trigger_condition)}">
        <span class="kb-sug-keyword">${highlightText(
          x.entry.trigger_condition.replace(/^关键字[:：]\s*/i, "").replace(/^咨询问题[:：]\s*/i, "❓").substring(0, 40),
          search
        )}</span>
        <span class="kb-sug-content">${highlightText(x.entry.reply_content.substring(0, 60), search)}</span>
        <span class="kb-sug-score">${x.score}分</span>
      </div>
    `
    )
    .join("");
  ...
  // 绑定点击事件：点击联想项直接填入搜索框并执行搜索
  sugEl.querySelectorAll(".kb-sug-item").forEach((el) => {
    el.addEventListener("click", () => {
      const searchInput = document.getElementById("kb-search");
      searchInput.value = el.dataset.keyword.substring(0, 50);
      sugEl.style.display = "none";
      kbPage = 1;
      applyKbFilter();
    });
    el.addEventListener("mousedown", (e) => e.preventDefault()); // 防止失焦
  });
}

// ── 自动同步触发 ────────────────────────────
async function triggerAutoSync(changedKey) {
  try {
    // 仅当用户已登录时触发自动同步
    if (typeof syncAuthManager !== 'undefined' && syncAuthManager.isLoggedIn()) {
      console.log(`[AutoSync] 检测到 ${changedKey} 变更，触发自动同步`);
      // 通过消息通知 background 执行同步
    ...
    console.log("[AutoSync] 自动同步跳过:", e.message);
  }
}
...
document.getElementById("kb-search").addEventListener("input", () => {
  kbPage = 1;
  const searchVal = document.getElementById("kb-search").value.trim().toLowerCase();
  // 显示联想建议
  renderSmartSuggestions(searchVal);
  applyKbFilter();
});
...
document.addEventListener("click", (e) => {
  const sugEl = document.getElementById("kb-suggestions");
  const searchInput = document.getElementById("kb-search");
  if (sugEl && searchInput && !searchInput.contains(e.target) && !sugEl.contains(e.target)) {
    sugEl.style.display = "none";
  }
});
...
function applyKbFilter() {
  ...
  kbFiltered = knowledgeBase.filter((e) => {
    ...
    if (search) {
      ...
      // 先用 fuzzyMatch 模糊匹配，再用 includes 精确匹配
  ...
  if (search) {
    kbFiltered.sort((a, b) => {
      const aCondition = (a.trigger_condition || "").toLowerCase();
      const bCondition = (b.trigger_condition || "").toLowerCase();
      const aContent = (a.reply_content || "").toLowerCase();
      const bContent = (b.reply_content || "").toLowerCase();

      // 关键词匹配优先
      const aKeyInclude = aCondition.includes(search) ? 10 : 0;
      const bKeyInclude = bCondition.includes(search) ? 10 : 0;
      const aContentInclude = aContent.includes(search) ? 5 : 0;
      const bContentInclude = bContent.includes(search) ? 5 : 0;

      return (bKeyInclude + bContentInclude) - (aKeyInclude + aContentInclude);
    });
  }
...
function renderKbList() {
  ...
  page.forEach((entry, displayIndex) => {
    ...
    // 关键词提取
    let keyword = entry.trigger_condition
      .replace(/^关键字[:：]\s*/i, "")
      .replace(/^咨询问题[:：]\s*/i, "❓")
      .replace(/^新的订单$/, "新订单")
      .substring(0, 50);
    ...
    div.innerHTML = `
      <div class="kb-item-header">
        <span class="kb-keyword" title="${escapeHtml(entry.trigger_condition)}">${search ? highlightText(keyword, search) : escapeHtml(keyword)}</span>
      ...
      <div class="kb-item-content">${search ? highlightText(entry.reply_content, search) : escapeHtml(entry.reply_content)}</div>
      ${entry.applicable_properties && entry.applicable_properties !== "全部"
        ? `<div class="kb-item-prop">📍 ${search ? highlightText(entry.applicable_properties.substring(0, 60), search) : escapeHtml(entry.applicable_properties.substring(0, 60))}</div>`
        : ""}
...
async function initSyncAuth() {
  ...
  if (cloudEndpointInput) {
    ...
    const syncConfigData = config.sync_config || {};
    if (syncConfigData.cloudEndpoint) {
      cloudEndpointInput.value = syncConfigData.cloudEndpoint;
    }
    cloudEndpointInput.addEventListener("change", async () => {
      const endpoint = cloudEndpointInput.value.trim();
      const saved = await chrome.storage.local.get('sync_config');
      const cfg = saved.sync_config || {};
      cfg.cloudEndpoint = endpoint;
      await chrome.storage.local.set({ sync_config: cfg });
      console.log("[Popup] 云端地址已保存:", endpoint);
    });
  }

  // 登录按钮 - 先保存云端地址
  document.getElementById("btn-sync-login")?.addEventListener("click", async () => {
    if (cloudEndpointInput?.value) {
      const endpoint = cloudEndpointInput.value.trim();
      const saved = await chrome.storage.local.get('sync_config');
      const cfg = saved.sync_config || {};
      cfg.cloudEndpoint = endpoint;
      await chrome.storage.local.set({ sync_config: cfg });
      // 同步更新 popup-sync.js 中的 syncConfig
      if (typeof syncConfig !== 'undefined') syncConfig.cloudEndpoint = endpoint;
    }
